#!/usr/bin/env python3
"""
M56b -- Paper 2B read-only repair audit:
...
def canonical_modality(value: Any, fallback_path: str = "") -> str:
    ...
    # M52 is the multimodal RFS repeated-nested analysis. Its OOF file has no
    # modality column, so path-based inference must recognize it explicitly.
    if (
        "m52_rfs_oof_predictions" in path_low
        or re.search(r"(^|[\\/])m52_rfs(?:_|[\\/])", path_low)
    ):
        return "Multimodal"
...
def audit_rfs_oof(
    ...
    for rel in oof_paths:
        ...
        for raw_modality, sub in groups:
            ...
            nonpositive_mask = num(s[tcol]) <= 0
            time_nonpositive = int(nonpositive_mask.sum())
            time_nonpositive_unique_patients = int(s.loc[nonpositive_mask, pid].nunique())
            ...
            try:
                ...
                # Evaluation-only exclusion of records with no positive follow-up.
                # This does not refit any model; it only checks whether such records
                # affect the already-saved OOF discrimination contrast.
                if time_nonpositive_unique_patients:
                    s_positive = s.loc[num(s[tcol]) > 0].copy()
                    pooled_positive, within_positive = repeat_pooled_delta(s_positive, schema)
                else:
                    pooled_positive, within_positive = pooled, within

                positive_time_change = (
                    pooled_positive - pooled
                    if np.isfinite(pooled_positive) and np.isfinite(pooled)
                    else math.nan
                )
                metric_status = "PASS"
                metric_error = ""
            except Exception as exc:
                pooled = within = pooled_positive = within_positive = math.nan
                positive_time_change = math.nan
            ...
            rec = {
                ...
                "time_le_0_unique_patients": time_nonpositive_unique_patients,
                "pooled_oof_mean_delta_c": pooled,
                "within_fold_weighted_mean_delta_c": within,
                "positive_time_only_pooled_delta_c": pooled_positive,
                "positive_time_only_within_fold_delta_c": within_positive,
                "positive_time_exclusion_change_delta_c": positive_time_change,
    ...
    for r in chosen_rows:
        ...
        details = (
            ...
            f"time<=0 unique patients={int(r.get('time_le_0_unique_patients', 0))}; "
            f"ΔC after evaluation-only t>0 exclusion="
            f"{r.get('positive_time_only_pooled_delta_c', math.nan):+.8f}; "
            f"source={r['source_path']}"
        )
        if (
            r["duplicate_repeat_patient_rows"]
            or r["inconsistent_patient_outcomes_across_repeats"]
            or (
                np.isfinite(r.get("positive_time_exclusion_change_delta_c", math.nan))
                and abs(r.get("positive_time_exclusion_change_delta_c", 0.0)) > 1e-10
            )
        ):
            status = "REVIEW"
...
def manuscript_rfs_check(
    ...
    for modality, ref in RFS_MANUSCRIPT_REFERENCES.items():
        ...
        # The manuscript's displayed point estimate is the direct repeated-OOF
        # point estimate. The percentile limits come from the saved patient
        # bootstrap. The bootstrap mean is an audit diagnostic, not the displayed
        # point estimate.
        diffs = {
            "delta": abs(point - ref["delta_c_index"]) if np.isfinite(point) else math.nan,
            "ci_low": abs(low - ref["ci_low"]) if np.isfinite(low) else math.nan,
            "ci_high": abs(high - ref["ci_high"]) if np.isfinite(high) else math.nan,
        }
        complete = all(np.isfinite(v) for v in diffs.values())
        status = (
            "PASS"
            if complete
        ...
        row = {
            ...
            "point_estimate_basis": "direct repeated-OOF point estimate",
            "interval_basis": "saved paired patient-bootstrap percentile interval",
            "status": status,
        ...
        audit.add(
            ...
            (
                f"reference={ref}; direct OOF point={point}; "
                f"saved bootstrap mean={boot_mean}; saved percentile CI=[{low},{high}]"
            ),
            True,
        )
...
def fold_ipcw_auc(
    ...
    schema: dict[str, str],
    horizon: float,
    exclude_nonpositive: bool = True,
) -> tuple[list[dict[str, Any]], list[str]]:
    """
    Cumulative/dynamic AUC within held-out folds.

    The censoring distribution is estimated from complement folds. Records with
    time <= 0 are excluded from this evaluation metric only.

    At a single horizon t, exact event/censoring times strictly after t are
    irrelevant to case/control membership. scikit-survival nevertheless requires
    all test follow-up times to lie below the largest training follow-up time.
    Therefore observations known to be event-free through t are represented as
    administratively censored just after t for the y_test object only. This
    preserves the case set (event <= t), control set (time > t), and all early
    censoring, while preventing an irrelevant extreme follow-up value from
    causing a fold to be discarded.
    """
    ...
    for fold_value in all_folds:
        ...
        test_n_before = len(test)
        train_n_before = len(train)
        ...
        train_time = num(train_surv[schema["time"]]).to_numpy(float)
        train_event = num(train_surv[schema["event"]]).fillna(0).to_numpy(int)
        max_train_time = float(np.max(train_time))

        if not (max_train_time > horizon):
            errors.append(
                f"fold={fold_value}: training censoring support ends at "
                f"{max_train_time}, not beyond horizon={horizon}"
            )
            continue

        eps = max(1e-6, abs(horizon) * 1e-8)
        clip_time = min(
            horizon + eps,
            float(np.nextafter(max_train_time, -np.inf)),
        )
        if not (clip_time > horizon):
            errors.append(
                f"fold={fold_value}: could not construct post-horizon test support "
                f"inside max training time={max_train_time}"
            )
            continue

        test_time_original = num(test_both[schema["time"]]).to_numpy(float)
        test_event_original = num(test_both[schema["event"]]).fillna(0).to_numpy(int)

        test_time_eval = test_time_original.copy()
        test_event_eval = test_event_original.copy()
        beyond = test_time_original > horizon
        test_time_eval[beyond] = clip_time
        test_event_eval[beyond] = 0

        y_train = make_surv(train_event, train_time)
        y_test = make_surv(test_event_eval, test_time_eval)
        ...
        details.append(
            {
                "fold": int(fold_value),
                "test_n_before_positive_time_filter": int(test_n_before),
                "train_n_before_positive_time_filter": int(train_n_before),
                "test_n": int(len(test_both)),
                "train_n": int(len(train_surv)),
                "test_nonpositive_time_excluded": int(test_n_before - len(test)),
                "train_nonpositive_time_excluded": int(train_n_before - len(train)),
                "post_horizon_test_records_admin_represented": int(beyond.sum()),
                "post_horizon_eval_time": float(clip_time),
                "max_training_followup_time": max_train_time,
                ...
                "legacy_delta_auc": (
                    legacy_m - legacy_c
                    if np.isfinite(legacy_c) and np.isfinite(legacy_m)
                    else math.nan
                ),
                "legacy_valid_n_clinical": legacy_n_c,
                "legacy_valid_n_model": legacy_n_m,
                "exclude_nonpositive_time": int(exclude_nonpositive),
            }
        )
...
def evaluate_auc_dataset(
    ...
    nonpositive_unique_patients = int(
        frame.loc[num(frame[schema["time"]]) <= 0, schema["patient"]].astype(str).nunique()
    )
    ...
    for repeat_value, rep in frame.groupby(schema["repeat"], sort=True):
        ...
        expected_folds_this_repeat = int(
            pd.to_numeric(rep[schema["fold"]], errors="coerce").nunique()
        )
        expected_total_folds += expected_folds_this_repeat

        if sksurv_available:
            details, errors = fold_ipcw_auc(
                rep,
                schema,
                HORIZON_MONTHS,
            ...
            if details:
                weights = np.asarray([d["test_n"] for d in details], dtype=float)
                ipcw_c = float(
                    np.average([d["ipcw_auc_clinical"] for d in details], weights=weights)
                )
                ipcw_m = float(
                    np.average([d["ipcw_auc_model"] for d in details], weights=weights)
                )
                ipcw_delta = ipcw_m - ipcw_c

                finite_c = [d for d in details if np.isfinite(d["legacy_binary_auc_clinical"])]
                finite_m = [d for d in details if np.isfinite(d["legacy_binary_auc_model"])]

                legacy_fold_c = (
                    float(
                        np.average(
                            [d["legacy_binary_auc_clinical"] for d in finite_c],
                            weights=[d["test_n"] for d in finite_c],
                        )
                    )
                    if finite_c
                    else math.nan
                ...
                legacy_fold_m = (
                    float(
                        np.average(
                            [d["legacy_binary_auc_model"] for d in finite_m],
                            weights=[d["test_n"] for d in finite_m],
                        )
                    )
                    if finite_m
                    else math.nan
                ...
                repeat_summaries.append(
                    {
                        ...
                        "legacy_fold_delta": (
                            legacy_fold_m - legacy_fold_c
                            if np.isfinite(legacy_fold_c) and np.isfinite(legacy_fold_m)
                            else math.nan
                        ),
                        ...
                        "successful_ipcw_folds": len(details),
                        "expected_ipcw_folds": expected_folds_this_repeat,
                    }
                )
    ...
    successful_total = int(repdf["successful_ipcw_folds"].sum())
    all_expected_repeats_present = bool(
        (repdf["successful_ipcw_folds"] == repdf["expected_ipcw_folds"]).all()
    )

    if not sksurv_available:
        ipcw_status = "NOT_VERIFIED"
    elif (
        len(errors_all) == 0
        and successful_total == expected_total_folds
        and all_expected_repeats_present
        and repdf["ipcw_clinical"].notna().all()
        and repdf["ipcw_model"].notna().all()
    ):
        ipcw_status = "PASS"
    elif repdf["ipcw_clinical"].notna().any():
        ipcw_status = "REVIEW"
    else:
        ipcw_status = "NOT_VERIFIED"
    ...
    result = {
        ...
        "nonpositive_time_unique_patients_excluded_from_ipcw": nonpositive_unique_patients,
        ...
        "legacy_foldweighted_binary_auc_clinical_mean": (
            float(repdf["legacy_fold_clinical"].mean())
            if repdf["legacy_fold_clinical"].notna().any()
            else math.nan
        ),
        "legacy_foldweighted_binary_auc_model_mean": (
            float(repdf["legacy_fold_model"].mean())
            if repdf["legacy_fold_model"].notna().any()
            else math.nan
        ),
        "legacy_foldweighted_binary_delta_auc_mean": (
            float(repdf["legacy_fold_delta"].mean())
            if repdf["legacy_fold_delta"].notna().any()
            else math.nan
        ),
        "ipcw_auc_clinical_mean": (
            float(repdf["ipcw_clinical"].mean())
            if repdf["ipcw_clinical"].notna().any()
            else math.nan
        ),
        "ipcw_auc_model_mean": (
            float(repdf["ipcw_model"].mean())
            if repdf["ipcw_model"].notna().any()
            else math.nan
        ),
        "ipcw_delta_auc_mean": (
            float(repdf["ipcw_delta"].mean())
            if repdf["ipcw_delta"].notna().any()
            else math.nan
        ),
        "expected_ipcw_fold_evaluations": int(expected_total_folds),
        "successful_ipcw_fold_evaluations": successful_total,
        "ipcw_error_count": len(errors_all),
        "ipcw_error_examples": " || ".join(errors_all[:5]),
        "ipcw_status": ipcw_status,
        "note": (
            "IPCW AUC is fold-contained cumulative/dynamic AUC. Censoring is estimated "
            "from complement folds within each repeat. time<=0 records are excluded from "
            "this evaluation metric only. Test outcomes strictly after the 60-month horizon "
            "are administratively represented just after 60 months for the metric's y_test "
            "object, preserving the 60-month case/control sets while satisfying censoring-"
            "support requirements. No prognostic model is refit."
        ),
    }
    ...
    audit.add(
        ...
        (
            ...
            f"successful/expected folds="
            f"{result['successful_ipcw_fold_evaluations']}/"
            f"{result['expected_ipcw_fold_evaluations']}; "
            f"time<=0 unique excluded={nonpositive_unique_patients}; "
        ...
        rows.append(
            {
                ...
                "analysis": model,
                "modality": ("RNA+CNV" if model == "clinical_rna_cna" else canonical_modality(model)),
...
def write_report(
    ...
    lines = [
        "# Paper 2B -- M56b RFS and censoring-aware AUC repair audit",
        "",
        f"**Status: {status}**",
        "",
        "## Scope",
    ...
    lines += [
        ...
        "- The IPCW AUC is fold-contained; time<=0 records are excluded from this evaluation metric only.",
        "- Follow-up strictly beyond 60 months is administratively represented just after 60 months in the test survival object; this preserves the 60-month case/control sets and prevents irrelevant tail-support failures.",
...
def make_share_zip(out: Path) -> Path:
    names = [
        ...
        "os_auc_fold_details.csv",
        "rfs_auc_comparison_optional.csv",
        "rfs_auc_fold_details_optional.csv",
        "inference_registry.csv",
        "console.log",
    ]
    zpath = out / "share_bundle_m56b.zip"
...
def main() -> int:
    ...
    out = report_parent / f"paper2b_m56b_{stamp}"
    ...
    with log_path.open("w", encoding="utf-8") as log:
        ...
        try:
            ...
            print("PAPER 2B / M56b / RFS + CENSORING-AWARE AUC REPAIR AUDIT")
