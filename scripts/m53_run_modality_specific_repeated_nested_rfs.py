...
def save_checkpoints(out, fold_rows, selected_rows, candidate_rows, predictions):
    atomic_write_csv(out / "m53_rfs_fold_checkpoint.csv", fold_rows)
    atomic_write_csv(out / "m53_rfs_selected_features_checkpoint.csv", selected_rows)
    atomic_write_csv(out / "m53_rfs_candidate_features_checkpoint.csv", candidate_rows)
    atomic_write_csv(out / "m53_rfs_oof_predictions_LOCAL_ONLY.csv", predictions)
...
def main() -> int:
    ...
    print("=" * 124)
    print("METABRIC M11.53 - RFS MODALITY-SPECIFIC REPEATED NESTED ANALYSIS")
    print("=" * 124)
    print("Endpoint: recurrence-free survival. Every supervised step is confined to the outer training fold.")
    ...
    fold_rows = read_rows(out / "m53_rfs_fold_checkpoint.csv")
    selected_rows = read_rows(out / "m53_rfs_selected_features_checkpoint.csv")
    candidate_rows = read_rows(out / "m53_rfs_candidate_features_checkpoint.csv")
    prediction_rows = read_rows(out / "m53_rfs_oof_predictions_LOCAL_ONLY.csv")
    completed = {
    ...
    for modality in modalities:
        ...
        time_values = pd.to_numeric(modality_clinical["rfs_months"], errors="coerce").to_numpy(dtype=float)
        event_values = pd.to_numeric(modality_clinical["rfs_event"], errors="coerce").to_numpy(dtype=int)
    ...
    write_csv(out / "m53_rfs_modality_cohort_summary.csv", cohort_rows)
    write_csv(out / "m53_rfs_modality_feature_universe.csv", universe_rows)
    ...
    write_csv(out / "m53_rfs_repeat_level_results.csv", repeat_rows)
    repeat_summary = summarize_repeats(repeat_rows, modalities)
    write_csv(out / "m53_rfs_repeat_level_summary.csv", repeat_summary)
    ...
    write_csv(out / "m53_rfs_feature_selection_frequency.csv", frequency_rows)
    write_csv(out / "m53_rfs_within_repeat_stability.csv", stability_rows)
    ...
    write_csv(out / "m53_rfs_modality_summary.csv", modality_summary)
