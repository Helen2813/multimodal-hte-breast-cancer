...
def save_checkpoints(
    ...
    atomic_write_csv(
        out / "m52_rfs_fold_checkpoint.csv",
        fold_rows,
    )
    atomic_write_csv(
        out / "m52_rfs_selected_features_checkpoint.csv",
        selected_rows,
    )
    atomic_write_csv(
        out / "m52_rfs_candidate_features_checkpoint.csv",
        candidate_rows,
    )
    atomic_write_csv(
...
def main() -> int:
    ...
    print("=" * 124)
    print("METABRIC M11.52 - RFS 20x5 RECONSTRUCTED NESTED TRACK B")
    print("=" * 124)
    print(
    ...
    print("Endpoint: recurrence-free survival (rfs_months / rfs_event).")
    ...
    if len(shared_samples) < int(settings["minimum_complete_n"]):
        raise RuntimeError("RFS Track B complete cohort below locked minimum.")
    ...
    if int(event.sum()) < int(settings["minimum_events"]):
        raise RuntimeError("RFS Track B event count below locked minimum.")

    fold_path = out / "m52_rfs_fold_checkpoint.csv"
    selected_path = out / "m52_rfs_selected_features_checkpoint.csv"
    candidate_path = out / "m52_rfs_candidate_features_checkpoint.csv"
    prediction_path = out / "m52_rfs_oof_predictions_LOCAL_ONLY.csv"
    ...
    write_csv(out / "m52_rfs_repeat_level_oof_results.csv", repeat_rows)
    ...
    write_csv(out / "m52_rfs_repeat_level_summary.csv", repeat_summary)
    ...
    write_csv(out / "m52_rfs_selection_frequency.csv", frequency_rows)
    ...
    write_csv(out / "m52_rfs_within_repeat_stability.csv", within_stability)
    ...
    write_csv(out / "m52_rfs_between_repeat_stability.csv", between_stability)
    ...
    write_csv(out / "m52_rfs_modality_composition.csv", composition_rows)

    summary = {
        "status": "FULL_RECONSTRUCTED_TRACK_B_RFS_COMPLETE",
    ...
    (out / "m52_rfs_track_b_full_summary.json").write_text(
    ...
    print("\nPASS: RFS full 20x5 reconstructed Track B completed.")
    return 0
