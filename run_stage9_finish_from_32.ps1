...
$GATE = Join-Path $ROOT "results\tables\31_landmark_paperA_gate.csv"
$RESULTS31 = Join-Path $ROOT "results\tables\31_landmark_aipw_results.csv"

foreach ($required in @($GATE, $RESULTS31)) {
...
$LOG = Join-Path $LOGDIR "stage9_finish_from32_$STAMP.log"

Start-Transcript -Path $LOG -Force

try {
    Write-Host "Stage 31 outputs found."
    Write-Host "Running Stage 32 preflight before any simulation."
    ...
    Write-Host ""
    Write-Host ("#" * 120)
    Write-Host "STAGE 9 FINISHED SUCCESSFULLY"
    Write-Host ("#" * 120)
    Write-Host "Review:"
